...
$(document).ready(function () {
  ...
  window.onkeydown = function (event) {
    ...
    //13 is the key number of enter, 32 is the key number for space, 91 for command.
    if (event.keyCode === 13) {
  ...
  var masterController = new Vue({
    ...
    methods: {
      ...
      askGPT3: function (prompt_in, question_in) {
        question_out = question_in + '\\n \\n AI:';
        console.log("question", question_out);

        let open_ai_response;

        async function openai_test() {

          var prompt_text = "PPretend that you are an autonomous vehicle and you were driving.";
          prompt_text += " Your goal is to explain and answer questions about what happened. Your answers should be concise. When a user asks for an explanation or asks any other questions, your responses should be informative and logical. You should always adhere to the technical information provided about the driving event. Do not makeup new technical information. If you don't find the answer in the technical information provided about the event, reply with 'I don't know' or 'I don't have that information at this time.’ Here is technical information about what happened while you were driving: " + prompt_in;
          //var prompt_text2 = "MORE TEXT HERE."

          //var url = "https://api.openai.com/v1/engines/text-davinci-003/completions";

          //var url = "https://api.openai.com/v1/completions";
          //var url = "https://xaistudyresource.openai.azure.com/";
          var url = "https://xaistudyresource.openai.azure.com/openai/deployments/testing/completions?api-version=2022-12-01 ";

          var xhr = new XMLHttpRequest();
          xhr.open("POST", url);

          xhr.setRequestHeader("Content-Type", "application/json");
          //xhr.setRequestHeader("Authorization", "Bearer 846a607a56c14a7080873343279ea99b");
          xhr.setRequestHeader("api-key", "5c2d2c83dd7b4b54a1ac67b9dd654385");
          xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
              ...
              console.log(text);
              masterController.$data.wizardAnswer = text;
              masterController.addChatEntry(question_in, text);

              userVehicleChat.push(question_in + '/*/' + masterController.$data.wizardAnswer);


              console.log("userVehicleChat", userVehicleChat);

            }
          };
          //"${prompt_text + question_out}"
          //start_sequence = "\nAI:"
          //estart_sequence = "\nHuman: "
          //console.log(JSON.parse(String.raw`"This is a double quote >> \" <<"`))
          prompt_text = prompt_text.slice(1);
          //question_out = question_out.slice(1);
          var our_input = prompt_text + question_out;
          our_input = our_input.replace(/"/g, '');
          //"prompt": "${prompt_text + question_out}",
          //our_input = JSON.stringify(our_input);
          //console.log(JSON.parse(String.raw`"{our_input}"`))
          //our_input =
          console.log("test test", our_input);
          var data = `{
          ...
          //data = data.Replace("{0}",prompt_text);
          console.log("here is the data", data);
          xhr.send(data);
        }
        console.log("we are in callGPT3 function:");
        openai_test();
        console.log("we are after the  callGPT3 function:");
        return masterController.$data.wizardAnswer;
      },
      noMoreQuestion: function () {
        ...
        document.getElementById("robot-container").style.width = "88%";

      },
      askWizard: function () {

        available_info = "I was Turning left,then  I Crashed. I had a Conflict with a lead vehicle. My crash was a typeII - Police-reportable Crash. I Braked (lockup)and I Skidded longitudinally. The summary of what happened is that I was driving along a dry, undivided business-area road during daylight hours.There is no adverse weather and traffic is moderate (LOS C). Vehicle 2 is directly ahead of me in a dedicated left turn lane leading to a stoplight.When the light changed, me and the vehicle ahead of me both performed a left turn.At this point, I got distracted by some internal processes. The vehicle ahead of me stoped and turned on its left turn signal, apparently intended to turn into an adjacent parking lot. Meanwhile, my attention was on the other inetrnal process. I noticed the vehicle in front of me stopped belatedly and rear-ended the lead vehicle at approximately 18 MPH, sending it forward a few feet. My judgment was Safe and legal. There are one passenger sitting in front and no passengers sitting in the back.\\n \\n Human:";
        typeof available_info;
        //masterController.$data.prompt_info = available_info;
        ...
        input = removeAll;
        masterController.$data.wizardAnswer = masterController.askGPT3(masterController.$data.prompt_info, input);
      ...
      processData: async function (dataa) {
        ...
        if (values.length < headings.length) {
          ...
          4 eventSeverity1
          5 crashSeverity1
          6 vehicle1EvasiveManeuver1
          7 vehicle1PostManeuver1
          8 eventNature2
          9 incidentType2
          10 eventSeverity2
          11 crashSeverity2
          12 vehicle1EvasiveManeuver2
          13 vehicle1PostManeuver2
          14 airbagDeployment
          15 vehicleRollover
          16 driverBehavior1
          17 driverBehavior2
          18 driverBehavior3
          19 frontSeatPassengers
          20 rearSeatPassengers
          21 vehicleContributingFactors
          22 visualObstructions
          23 lighting
          24 weather
          25 surfaceCondition
          26 trafficFlow
          27 contigTravelLanes
          28 throughTravelLanes
          29 infrastructure
          30 vehicle1LaneOccupied
          31 trafficDensity
          32 trafficControl
          33 relationToJunction
          34 alignment
          35 constructionZone
          36 numberOfOtherMotorists
          37 numberOfObjectsAnimals
          38 fault
          39 motorist2Location
          40 motorist2Type
          41 motorist2Maneuver
          42 motorist2Reaction
          43 motorist3Location
          44 motorist3Type
          45 motorist3Maneuver
          46 motorist3Reaction
          47 finalNarrative
          48 intersectionInfluence
          49 preIncidentManeuver
          50 maneuverJudgment
          51 explanation*/
          /*
          "airbag Deployment:" ['Not Applicable', 'No']
"vehicle Rollover:" ['Not Applicable', 'No']
"vehicle ContributingFactors:" ['None']
"visual Obstructions:" ['Rain, snow, fog, smoke, sand, dust', 'Sunlight', 'No obstruction interfered with the autonomous vehicle’s ability to drive', 'Curve or hill']
"lighting:" ['Darkness, not lighted', 'Darkness, lighted', 'Daylight', 'Dusk']
"pre Incident Maneuver:" ['Decelerating in traffic lane', 'Going straight, constant speed', 'Going straight, accelerating', 'Passing or overtaking another vehicle', 'Negotiating a curve', 'Going straight, but with unintentional drifting within lane or across lanes', 'Turning right', 'Changing lanes', 'Turning left', 'Making U-turn', 'Starting in traffic lane', 'Stopped in traffic lane'] ---------------------
"event Severity1:" ['Near-Crash'] ---------------------
"event Nature1:" ['Conflict with vehicle turning into another vehicle path (opposite direction)', 'Conflict with a lead vehicle', 'Conflict with vehicle in adjacent lane', 'Conflict with vehicle turning across another vehicle path (opposite direction)', 'Conflict with oncoming traffic', 'Conflict with pedestrian', 'Conflict with vehicle turning into another vehicle path (same direction)', 'Conflict with obstacle/object in roadway', 'Single vehicle conflict'] ---------------------
"crash Severity1:" ['Not a Crash'] ---------------------
"vehicle1 Evasive Maneuver1:" ['Steered to left', 'Braked and steered right', 'Braked (lockup)', 'Released brakes', 'Braked and steered left', 'Steered to right', 'Braked (no lockup)', 'No reaction'] ---------------------
"vehicle1PostManeuver1:" ['Skidded longitudinally', 'Continued to do the same as before the incident', 'Control maintained'] ---------------------
"front Seat Passengers:" [1, 2] ---------------------
"rear Seat Passengers:" [0, 1, 2] ---------------------
"weather:" ['Moderate/Heavy Rain', 'Mist/Light Rain', 'Snowing', 'No Adverse Conditions'] ---------------------
"surface Condition:" ['Dry', 'Wet', 'Snowy'] ---------------------
"traffic Flow:" ['Divided (median strip or barrier)', 'Not divided - simple 2-way trafficway', 'Not divided - center 2-way left turn lane', 'No lanes', 'One-way traffic'] -------------haven't add this one--------
"contig Travel Lanes:" [0, 1, 2, 3, 4, 5] -----------haven't add this one----------
"traffic Density:" ['Free flow, leading traffic present', 'Flow with some restrictions', 'Level-of-service B: Flow with some restrictions', 'Level-of-service A2: Free flow, leading traffic present', 'Free flow, no lead traffic', 'Level-of-service A1: Free flow, no lead traffic', 'Unstable flow - temporary restrictions substantially slow driver', 'Level-of-service C: Stable flow, maneuverability and speed are more restricted', 'Stable flow, maneuverability and speed are more restricted'] ---------------------
"traffic Control:" ['No traffic control', 'Stop sign', 'Traffic lanes marked', 'Construction signs/warnings', 'Railroad crossing with markings or signs', 'Traffic signal'] ---------------------
"relation ToJunction:" ['Intersection', 'Parking lot entrance/exit', 'Interchange area', 'Intersection-related', 'Parking lot, within boundary', 'Non-junction'] ---------------------
"alignment:" ['Straight', 'Curve left', 'Curve right'] ---------------------
"construction Zone:" ['Not construction zone-related', 'Construction zone-related (occurred in approach or otherwise related to zone)', 'Construction Zone (occurred in zone)'] ---------------------
"number Of Other Motorists:" [0, 1, 2] ---------------------
"number Of Objects Animals:" [0, 1] ---------------------
"fault:" ['Driver of the other vehicle', 'Subject driver', 'The autonomous vehicle', 'Driver 2'] ---------------------
"intersection Influence:" ['Yes, Stop Sign', 'Yes, Parking lot, Driveway Entrance/Exit', 'Yes, Uncontrolled', 'No', 'Yes, Other', 'Yes, Interchange', 'Yes, Traffic Signal'] ---------------------
"maneuverJudgment:" ['Unsafe but legal', 'Unsafe and illegal', 'Safe and legal'] ---------------------
          */
          //5.	How do you describe the lighting in the given driving event?( 'Daylight', 'Darkness, lighted', 'Dusk', 'Darkness, not lighted', 'Dawn')
          ...
          let all_vals_q6 = ['Decelerating in traffic lane', 'Going straight, constant speed', 'Going straight, accelerating', 'Passing or overtaking another vehicle', 'Negotiating a curve', 'Going straight, but with unintentional drifting within lane or across lanes', 'Turning right', 'Changing lanes', 'Turning left', 'Making U-turn', 'Starting in traffic lane', 'Stopped in traffic lane'];
          ...
          let all_vals_q10 = ['Control maintained', 'Continued to do the same as before the incident', 'Skidded longitudinally', 'Rotated clockwise'];
          ...
          let all_vals_q11 = ['No Adverse Conditions', 'Moderate/Heavy Rain', 'Fog', 'Snowing', 'Mist/Light Rain'];
          ...
          let all_vals_q20 = ['Not applicable', 'Unable to determine', 'Driver of the other vehicle', 'The autonomous vehicle']
          ...
          let all_vals_q24 = ['Because the autonomous vehicle chose to drive for fun and inflicted an unsafe situation.', 'Because the autonomous vehicle conducted a rolling stop and turned too widely.', 'Because another vehicle incorrectly turned towards the autonomous vehicle.', 'Because the autonomous vehicle passed the vehicle in the wrong lane and did not allow enough room for it to properly merge.'];
      ...
      submitTask: function () {
        ...
        */
        if (masterController.checkUserInput("q1After") || masterController.checkUserInput("q2After") || masterController.checkUserInput("q3After") || masterController.checkUserInput("q4After") || masterController.checkUserInput("q5After") || masterController.checkUserInput("q6After") || masterController.checkUserInput("q7After") || masterController.checkUserInput("q8After") || masterController.checkUserInput("q9After") || masterController.checkUserInput("q10After") || masterController.checkUserInput("q11After") || masterController.checkUserInput("q12After") || masterController.checkUserInput("q13After") || masterController.checkUserInput("q14After") || masterController.checkUserInput("q15After") || masterController.checkUserInput("q16After") || masterController.checkUserInput("q17After") || masterController.checkUserInput("q18After") || masterController.checkUserInput("q19After") || masterController.checkUserInput("q20After") || masterController.checkUserInput("q21After")) {
          ...
          console.log("here is the content:", userVehicleChat);
